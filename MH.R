#---------------phi update------------------#
Update_phis = function(model_s, data){
  ...
  for(a in a_exc0){
    ...
    model_s = phi_accept_reject(proposed_model = proposed_model, model_s = model_s, data = data)
...
phi_accept_reject = function(proposed_model, model_s, data){
...
#-------------- alloc_A update --------------#
Update_allocations_A = function(model_s, data, scramble){
  
  model_s = new_allocations_A(model_s = model_s, data = data)
...
new_allocations_A = function(model_s, data){
  ...
  for(i in 2:model_s$n){
    if(runif(1,0,1) < model_s$q_A){
      ...
      q = marginal_post_w(model_s = model_s, data = data, i = i)
...
marginal_post_w = function(model_s, data, i){
  
  q = rep(NA, model_s$postCl_df_A+1)
  
  for(a in 1:(model_s$postCl_df_A+1)){
    model_s$postAllocation_A[i] = a
    q[a] = llh_pairwise_mcmc_single(model_s = model_s, data = data, i = i) + prior_DMA_A(model_s = model_s)
...
#------------- theta update -------------------#
Update_thetas_A = function(model_s, data){
  ...
  if(model_s$postCl_df != 0){# if ony 0 cluster exists then don't update thetas (obviously)
    for(k in 1:model_s$postCl_df){
      ...
      model_s = theta_accept_reject_A(proposed_model = proposed_model, model_s = model_s, data = data, k=k)
...
theta_accept_reject_A = function(proposed_model, model_s, data, k){
...
#--------------- update allocations -----#
Update_allocations_ = function(model_s, data){
  
  model_s = new_allocations_(model_s = model_s, data = data)
...
marginal_post_y_ = function(model_s, data, i, j){
  ...
  for(k in -model_s$postCl_df:model_s$postCl_df){
    ...
    q[b] = llh_pairwise_mcmc_pair_A(model_s = model_s, data = data, i = i, j = j) + prior_DMA(model_s = model_s)
    b = b+1
  }
  
  return(q)
}
new_allocations_ = function(model_s, data){
  ...
  for(i in 2:(n-1)){
    for(j in (i+1):(n)){
        ...
        q = marginal_post_y_(model_s = model_s, data = data, i = i, j = j)
        q_ = exp(q)
        if(any(exp(q) == 0)){
          stop("error: some probabilities are 0")
        }
